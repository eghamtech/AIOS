...
class cls_agent_{id}:
    ...
    def run(self, mode):
        ...
        np_column = self.np.array(df[self.col1]).reshape(-1, 1)
        # find max and min values ignoring NaN and INF values
        np_column_max = self.np.ma.masked_invalid(np_column).max()
        np_column_min = self.np.ma.masked_invalid(np_column).min() 
        # replace INF and -INF with above
        np_column[np_column == self.inf] = np_column_max
        np_column[np_column == -self.inf] = np_column_min

        # replace NaN with mean values
        np_column = self.Imputer().fit_transform(np_column)

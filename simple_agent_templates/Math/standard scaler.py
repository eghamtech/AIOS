...
#key=ignore_columns_containing;  type=constant;  set='%ev_field%' and '%scaled_%'
#key=field_prefix;  type=random_from_set;  set=scaled_
#end_of_parameters
...
class cls_agent_{id}:
    ...
    def __init__(self):
        ...
        for i in range(1,4):
            output_column = self.agent_name + '_' + self.field_prefix + self.col1 + self.sfx[i-1] + str(self.result_id)
    ...
    def run(self, mode):
        ...
        for i in range(1,4):
            ...
            output_column   = self.field_prefix + self.col1 + self.sfx[i-1] + str(self.result_id)
            ...
            scaler_filename = self.agent_name + '_' + output_column + ".model"

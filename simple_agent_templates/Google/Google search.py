...
if count_non_empty<=1:
    ...
    for el in df.loc[df['text_for_google'].isnull()==False, 'text_for_google'].unique():
        #print ("work with: ", el[:20]+"...")
        if len(el)>0:
            try:
                html = requests.get("http://google.com/search?q=" + urllib.parse.quote_plus(el[:255])).text
                doc = fromstring(html)
        ...
        if idx>=block:
            idx=0
            print("Google processing item "+str(count)+" of "+str(ntotal))

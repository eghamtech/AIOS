...
class cls_ev_agent_{id}:
    ...
    def set_seed(self, seed_init):
        # The below is necessary for starting Numpy and core Python generated random numbers
    ...
    def model_params(self):
        self.params['algo']['optimizer']                = {optimizer}
        self.params['algo']['activation']               = {activation}
        self.params['algo']['layers']                   = {layers}
        self.params['algo']['neurons']                  = {neurons}
        self.params['algo']['batch_size']               = {batch_size}
        self.params['algo']['epochs']                   = {epochs}
        self.params['algo']['dropout']                  = {dropout}
        self.params['algo']['activation_output']        = {activation_output}
        self.params['algo']['early_stopping_min_delta'] = {early_stopping_min_delta}

        self.params['random_seed']                  = self.rn_seed_init     
        self.params['num_threads']                  = self.num_threads
        ...
        if self.is_binary:
            ...
            self.params['algo']['s_loss_function']             = 'binary_crossentropy'
            ...
            self.params['algo']['early_stop_metric']           = 'val_tf_prc_auc'
            self.params['algo']['early_stop_metric_direction'] = 'max'
            ...
            self.params['algo']['early_stop_metric']           = 'val_loss'
            self.params['algo']['early_stop_metric_direction'] = 'auto'
        else:
            print ("detected regression target: use RMSE/MAE")
            self.params['algo']['objective']                   = self.objective_regression
            self.params['algo']['s_loss_function']             = 'mean_squared_error'
    ...
    def model_init(self):
        ...
        with tf.device(self.s_tf_device):
            ...
            for i in range(self.params['algo']['layers']):
                if i == 0:
                    mlp_model.add(Dense(self.params['algo']['neurons'], activation=self.params['algo']['activation'], input_dim=self.params['algo']['input_dim']))
                else:
                    mlp_model.add(Dense(self.params['algo']['neurons'], activation=self.params['algo']['activation']))

                mlp_model.add(Dropout(self.params['algo']['dropout']))

            # add output layer
            mlp_model.add(Dense(self.params['algo']['num_class'], activation=self.params['algo']['activation_output']))

            mlp_model.compile(loss=self.params['algo']['s_loss_function'], optimizer=self.params['algo']['optimizer'], metrics=self.metric)
    ...
    def model_save(self, predictor, file_path):
        predictor['ml_model'].save(file_path)
    ...
    def model_train(self, ml_model, x_train, y_train, x_test, y_test, current_fold):
        ...
        with tf.device(self.s_tf_device):
            ...
            self.print_html(pd.DataFrame(mlp_history.history))

...
#key=field_ev_prefix;  type=random_from_set;  set=ev_field_trnlp_applied_
...
#key=transformer_model;  type=random_from_set;  set='gpt2','ctrl','openai-gpt','xlnet','transfo-xl','xlm'
#key=transformer_model_id;  type=random_int;  from=0;  to=9;  step=1
#key=transformer_length;  type=random_int;  from=10;  to=20;  step=1
#key=transformer_temperature;  type=random_float;  from=0;  to=2;  step=0.1
#key=transformer_rept_penalty;  type=random_float;  from=0;  to=2;  step=0.1
#key=transformer_top_k;  type=random_int;  from=0;  to=100;  step=1
#key=transformer_top_p;  type=random_float;  from=0;  to=2;  step=0.1
...
GEN_ALL_MODELS = sum((tuple(conf.pretrained_config_archive_map.keys()) for conf in (GPT2Config, OpenAIGPTConfig, XLNetConfig, TransfoXLConfig, XLMConfig, CTRLConfig)), ())
...
class cls_ev_agent_{id}:
    ...
    def model_env_init(self):   
        ...
        n_model_names   = len(GEN_MODEL_NAMES[self.transformer_model])
        mappd_name_id   = self.map_from_continuous_set_to_set(10, n_model_names, self.model_name_id)  # assuming 10 random options in "transformer_model_id" - change if expanded
        self.model_name = GEN_MODEL_NAMES[self.transformer_model][mappd_name_id]
        
        nlp_model_config, nlp_model_class, nlp_tknzr_class = GEN_MODEL_CLASSES[self.transformer_model]

        self.nlp_tknzr = nlp_tknzr_class.from_pretrained(self.model_name)
        self.nlp_model = nlp_model_class.from_pretrained(self.model_name)
    ...
    def model_load(self, file_path):
        predictor = lgb.Booster(model_file=file_path)
        return predictor
    ...
    def convert_df_to_nlp_token_ids(self, df_data):       
        xlm_lang = None
        # XLM Language usage detailed in the issues #1414
        if self.transformer_model in ["xlm"]:
            xlm_lang = self.nlp_tknzr.lang2id['en']  # english only for now

        # XLM masked-language modeling (MLM) models need masked token (see details in sample_sequence)
        is_xlm_mlm = self.transformer_model in ["xlm"] and 'mlm' in self.model_name
        if is_xlm_mlm:
            xlm_mask_token = self.nlp_tknzr.mask_token_id
        else:
            xlm_mask_token = None
        
        is_xlnet = bool(self.transformer_model == "xlnet")
        
        length             = {transformer_length}
        temperature        = {transformer_temperature}
        repetition_penalty = {transformer_rept_penalty}
        top_k              = {transformer_top_k}
        top_p              = {transformer_top_p}
        
        all_token_ids = []
        
        if self.target_col in df_data.columns:
            df_x = df_data.drop(self.target_col, 1)
        else:
            df_x = df_data.copy()
        ...
        for i, rowTuple in enumerate(df_x.itertuples(index=False)):
            ...
            pred_tokens = self.sample_sequence( model=self.nlp_model, context=context_tokens, length=length,
                                   temperature=temperature, top_k=top_k, top_p=top_p, repetition_penalty=repetition_penalty,
                                   is_xlnet=is_xlnet, is_xlm_mlm=is_xlm_mlm, xlm_mask_token=xlm_mask_token, xlm_lang=xlm_lang,
                                   device=self.device )

            pred_tokens = pred_tokens[0, len(context_tokens):].tolist()
        ...
        for n in range(length):
            new_cols.append(self.field_ev_prefix + '_pt' + str(n))
            
        df_x = pd.DataFrame(all_token_ids, columns=new_cols)
        
        if self.target_col in df_data.columns:
            df_x[self.target_col] = df_data[self.target_col]
...
#                     df_col[col_name] = df_col[col_name].map(dict1)                                                    # map and replace
        ...
        # text needs to be converted to tokens by Transformer NLP models
        df = self.convert_df_to_nlp_token_ids(df)
    ...
    def run(self, mode):
        ...
        df_all = self.convert_df_to_nlp_token_ids(df_all)

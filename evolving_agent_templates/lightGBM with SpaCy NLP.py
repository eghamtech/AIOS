...
#key=field_ev_prefix;  type=random_from_set;  set=ev_field_lgbm_spacy_
...
class cls_ev_agent_{id}:
    ...
    def convert_df_to_row_embeddings(self, df_data):
        # concatenate all fields in df_data into one string
        # and convert into vector embeddings for each string (row of fields)
        df_data_l = []
        ...
        for i, rowTuple in enumerate(df_data.itertuples(index=False)):
            ...
            doc = spacy_nlp(row)          # tokenize row using SpaCy
            row_v = []
            for t in doc:
                row_v.append(t.vector)    # extract embedding vector for each token
            row_v_np = np.array(row_v)    # and convert list of embedding vectors into array

            svd = TruncatedSVD(n_components=1, random_state=self.rn_seed_init)
            svd.fit(row_v_np)             # get principal components 
            svd = svd.components_

            row_v_np = row_v_np - row_v_np.dot(svd.transpose()) * svd   # subtract principal component
            row_v_np = np.mean(row_v_np, axis=0)                        # mean by columns creating single aggreggate vector of the whole row of fields

            df_data_l.append(list(row_v_np))
    ...
    def model_train(self, ml_model, x_train, y_train, x_test, y_test, current_fold):
        print (str(datetime.now()), " SpaCy train data Fit Transform")
        x_train = self.convert_df_to_row_embeddings(x_train)  
        
        print (str(datetime.now()), " SpaCy test data Transform")
        x_test  = self.convert_df_to_row_embeddings(x_test)
    ...
    def model_predict(self, predictor, xt, fold=-1, mode=0):
        try:
            print (str(datetime.now()), " SpaCy predict data transform")
            xt = self.convert_df_to_row_embeddings(xt)
